...
## Data preparation
gc = pygsheets.authorize(service_account_file='calculator/data/lasso/certification/hackthon-396711-57515e0c836f.json')
survey_key = '1lwyWwhDdzcbvOCVUzKuJac8oRRysWfScTdOHfzXsTME'
sh = gc.open_by_key(survey_key)
ws = sh.worksheet_by_title('工作表1')
ws.export(path= 'calculator/data/lasso/', filename = 'data')

df = pd.read_csv('calculator/data/lasso/data.csv', index_col=False)
...
def get_ratio(input_data):
  return LassoReg(data_changed, y, input_data)[0]